...
const updateAvailableFilters = (_) => {
  filters.types.forEach((type) => {
    const availableFilters = flatten(filtersAvailableFor(type))
      .filter(onlyUnique)
    ...
    availableFilters.forEach((filter) =>
      enableButton(get("option", filter, type))
    );
    unavailableFilters.forEach((filter) =>
      disableButton(get("option", filter, type))
    );
  });
};
...
const byIndustry = (client) => {
  ...
  return (
    industryFilters.length === 0 || industryFilters.includes(clientIndustry)
  );
};
