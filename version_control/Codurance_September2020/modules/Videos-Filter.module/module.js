class VideosFilter {
  ...
  getAll = (entity, type) => {
    ...
    return Array.from(document.querySelectorAll(selectors[entity]));
  };
  ...
  setFilterOptions = (_) => {
    this.filters.types.forEach(
      (type) => (this.filters.all[type] = this.filterOptions(type))
    );
  };
  ...
  addFilterToggleListener = (_) => {
    ...
    filterToggleButton.addEventListener("click", this.toggleShowHideFilters);
  };
  ...
  addSearchBarListeners = () => {
    this.get("search_bar_input").addEventListener(
      "keypress",
      this.dismissEnterKey
    );

    this.get("search_bar_input").addEventListener(
      "input",
      this.filterVideosOnInputValueChange
    );

    this.get("search_bar_reset_button").addEventListener(
      "click",
      this.filterVideosOnResetButtonClick
    );
  };
  ...
  hideDropdown = (container) => {
    ...
    this.get("dropdown_icon", type).classList.remove(
      "filter-dropdown-icon--selected"
    );
  };
  ...
  updateAvailableFilters = () => {
    this.filters.types.forEach((type) => {
      const availableFilters = this.filtersAvailableFor(type)
        .flat(Infinity)
        .filter(this.onlyUnique)
        .filter((element) => element.trim().length > 0);

      const unavailableFilters = this.arrayDifference(
        this.filters.all[type],
        availableFilters
      );
  ...
  filterSelected = (type, value) => {
    this.filterAlreadyApplied(type, value)
  ...
  addRemoveFilterListener = (type, button) => {
    button.addEventListener("click", (_) => {
      const value = button.dataset[`remove${this.capitalise(type)}Filter`];
      this.removeFilter(type, value);
    });
  };
  ...
  dismissEnterKey = (keypressEvent) => {
    ...
    if (keypressEvent.keyCode == 13) keypressEvent.preventDefault();
  };
  ...
  togglePromotedVideos = (searchBarText) => {
    ...
    const isSearchBarEmpty =
      searchBarText === "" || searchBarText === undefined;
  ...
  toggleSearchResultsTitle = (searchBarText) => {
    const generalTitle = document.querySelector(
      ".videos .card-collection__title"
    );
    const searchResultsTitle = document.querySelector(
      ".videos .card-collection__search-results-title"
    );
  ...
  showSearchBarResetButton = () => {
    this.get("search_bar_form").classList.add("videos-search-bar--icon-hidden");
    this.removeHiddenModifier(this.get("search_bar_reset_button"));
  };

  hideSearchBarResetButton = () => {
    this.get("search_bar_form").classList.remove(
      "videos-search-bar--icon-hidden"
    );
  ...
  showWithAnimation = (element) => {
    ...
    setTimeout(this.removeFadeAnimationModifier, animationDuration, element);
  };
  ...
  hideWithAnimation = (element) => {
    ...
    // Timeout to show the transition before the display property changes
    setTimeout(this.addHiddenModifier, animationDuration, element);
  };
