...
const appendLocations = (entry, jobData) => {
  const newLocation = {
    ...
    location: hasHybridCity(jobData.city)
      ? jobData.city
      : getDeterminedCountry(jobData.country)
  };
...
const filterUniqueLocations = (data) => {
  return [
    ...new Set(
      data.map((job) =>
        hasHybridCity(job.city) ? job.city : getDeterminedCountry(job.country)
      )
    )
  ];
};
...
const displayDropdownButtons = (filteredItems, idSelector) => {
  const htmlItems = filteredItems
    .map((item) => {
      return `
      <li>
        <input type="checkbox" id="${convertToDashCase(
          item
        )}" class="jobs__filter-dropdown-option" name="${item}" data-category="${idSelector}"/>
...
const renderLocations = (locationsArray) => {
  return locationsArray
    .map(
      ({ city, country, url, location, workType }) => `
...
const displayJobs = (jobsData) => {
  ...
  const htmlRender = Object.entries(jobs)
    .map(([departmentName, jobInfo]) => {
      return ` 
      <div class="job-item__section">
        <p class="job-item__department">${departmentName}</p>
          ${Object.entries(jobInfo)
            .map(([jobTitle, { locations }]) => {
              return `
            <div class="job-item__titles-container">
              <h3 id=${convertToPascalCase(
                jobTitle
              )} class="job-item__title">${jobTitle}</h3>
...
const closeAllDropdowns = () => {
  ...
  allContainers.forEach((element) => {
    element.children[0].classList.remove(
      "jobs__filter-dropdown-button--active"
    );
...
const hideEmptySections = () => {
  ...
  allSections.forEach((section) => {
    const jobTitles = Array.from(
      section.querySelectorAll(".job-item__titles-container")
    );
...
const checkEmptyResults = () => {
  const allSections = Array.from(
    document.querySelectorAll(".job-item__section")
  );
...
const allItemsAreHidden = (arr) =>
  arr.every((el) => el.classList.contains("hidden"));
...
const renderFilteredResults = (filterState) => {
  ...
  for (const jobListing of jobListings) {
    ...
    const jobPositions = Array.from(
      jobListing.querySelectorAll(".job-list__position")
    );

    if (
      filterIncludes(filterState.roles, jobTitle) ||
      filterIsEmpty(filterState.roles)
    ) {
      show(jobListing);
    } else {
      hide(jobListing);
    ...
    for (const jobPosition of jobPositions) {
      const jobLocation = jobPosition
        .querySelector(".job-item__location")
        .innerText.trim();
      const jobWorkType = jobPosition
      ...
      if (
        ...
        (filterState.workType === jobWorkType || filterState.workType === "All")
      ) {
        show(jobPosition);
      } else {
        hide(jobPosition);
...
const updateFilterState = async (
  { target: { type, checked, dataset, name, value } },
  filterState
) => {
  switch (type) {
    case "checkbox": {
      if (checked) {
        filterState[dataset.category] = addElementToArray(
          filterState[dataset.category],
          name
        );
      } else {
        filterState[dataset.category] = removeElementFromArray(
          filterState[dataset.category],
          name
        );
      }
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  filterForm.addEventListener("change", (e) =>
    handleFilterFormChange(e, filterState)
  );
});
